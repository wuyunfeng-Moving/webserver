...
import type { Message, ServerResponse, Blog, User } from '../shared/types.js';
...
// 文件系统管理类
class FileStorage<T extends { id: string }> {
    ...
    public addData(item: T) {
        this.data.push(item);
        this.saveData();
    }

    public updateData(id: string, newData: Partial<T>) {
...
// 使用存储类
const messagesStorage = new FileStorage<Message>(messagesFilePath);
const blogsStorage = new FileStorage<Blog>(blogsFilePath);
const usersStorage = new FileStorage<User>(usersFilePath);
...
app.post('/register', (req, res: ExpressResponse) => {
    try {
